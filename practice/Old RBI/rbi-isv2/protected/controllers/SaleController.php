...
class SaleController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Sale']))
		{
			...
			//if($model->save())
				...
				//$this->redirect(array('saleDetail/create','sale_id'=>$model->id));
				
				if($model->save()){
					$old = new Logs;
					$old->log_date_time=date('Y-m-d H:i:s');
					$old->log_action="Created new sale: Sale #" .
					$model->id . ", " . $model->sale_officialreceipt_no;
					$old->log_module="Sale";
					$old->user_id= Yii::app()->user->id;
					$this->redirect(array('saleDetail/create','sale_id'=>$model->id));
					if($old->save()) $this->redirect(array('saleDetail/create','sale_id'=>$model->id));
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['Sale']))
		{
			...
			$old = new Logs;
			$old->log_date_time=date('Y-m-d H:i:s');
			$old->log_action="Updated sale #" . $model->id . ", " . $model->sale_officialreceipt_no .".";
			$old->log_module="Sale";
			$old->user_id= Yii::app()->user->id;
	...
	public function actionDelete($id)
	{
		...
		$old = new Logs;
		$old->log_date_time=date('Y-m-d H:i:s');
		$old->log_action="Deleted the sale, " . $model->sale_officialreceipt_no . ".";
		$old->log_module="Sale";
		$old->user_id= Yii::app()->user->id;
		$old->save();
